            ...
            case CODE_WEATHER:
                ...
                try{
                    for(ContentValues contentValue : values){
                        long weatherDate = contentValue.getAsLong(WeatherContract.WeatherEntry.COLUMN_DATE);
                        if(!SunshineDateUtils.isDateNormalized(weatherDate)){
                            throw new IllegalArgumentException("Date must be normalized to insert");
                        }

                        long _id = db.insert(WeatherContract.WeatherEntry.TABLE_NAME, null, contentValue);
                        if(_id != -1){
                            rowsInserted++;
                        }
                    }
